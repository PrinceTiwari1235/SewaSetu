  ...
  const metricCards = [
    {
      ...
      icon: Users,
      color: "text-blue-400",
      bg: "bg-blue-950/50   border-blue-900/30",
    },
    {
      ...
      bg: "bg-crimson-950/50 border-crimson-900/30",
    },
    {
    ...
    },
    {
      ...
      bg: "bg-green-950/50  border-green-900/30",
    },
  ];
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {metricCards.map((card) => {
          ...
          return (
            <div
              key={card.label}
              className={`glass-card p-5 border ${card.bg}`}
            >
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-white font-semibold text-sm">
              Most Bookmarked Portals
            </h2>
            <Link
              href="/admin/portals"
              className="text-crimson-400 hover:text-crimson-300 text-xs transition-colors"
            >
              Manage all →
            </Link>
          </div>
          <div className="glass-card divide-y divide-white/[0.04]">
            {topPortals.map((portal, i) => (
              ...
              >
                <span className="text-white/20 text-xs w-4 text-center">
                  {i + 1}
                </span>
                <span className="text-lg flex-shrink-0">{portal.icon}</span>
                <div className="flex-1 min-w-0">
                  <div className="text-white text-sm truncate group-hover:text-crimson-300 transition-colors">
                    {portal.name}
                  </div>
        ...
        <div>
          ...
          <div className="glass-card divide-y divide-white/[0.04]">
            {recentUsers.map((user) => (
              ...
              >
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0 ${user.role === "ADMIN" ? "bg-crimson-700" : "bg-mountain-700"}`}
                >
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  {user.role === "ADMIN" && (
                    <span className="text-[10px] bg-crimson-950/60 border border-crimson-800/40 text-crimson-400 px-1.5 py-0.5 rounded">
                      Admin
                    </span>
                  )}
